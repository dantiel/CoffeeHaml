...
class EmitState {
  ...
  hoisted: string[] = []; // statement JS hoisted before return
  directEmit: boolean = false; // emit statements inline (wrap='none')
...
function compileComponentBody(ast: Document, state: EmitState): string {
  ...
  // Transfer warnings and hoisted statements from temp state
  state.warnings.push(...tmpState.warnings);

  const body = tmpState.output.trim();

  // Hoisted statements (from - blocks) go before the return
  const hoistedJs = tmpState.hoisted.length > 0
    ? tmpState.hoisted.join('; ') + '; '
    : '';

  if (!body) {
    if (hoistedJs) return `(() => { ${hoistedJs}return null; })()`;
    return 'null';
  }
  ...
  const jsx = stmts.length === 0 ? 'null'
    : stmts.length === 1 ? stmts[0]
...
function emitElement(el: Element, state: EmitState): void {
  ...
  // Attributes
  const attrResult = buildAttributes(el);
  const attrs = attrResult.parts;
  const keyArg = attrResult.keyExpr ? `, ${attrResult.keyExpr}` : '';
  const attrsJs = attrs.length > 0 ? `{ ${attrs.join(', ')} }` : 'null';

  // Children — embedded in props per React JSX runtime convention
  if (hasChildren) {
    if (el.children.length === 1) {
      const childJs = emitChildToJs(el.children[0], state);
      const childrenAttr = `children: ${childJs}`;
      const allAttrs = attrs.length > 0 ? `{ ${attrs.join(', ')}, ${childrenAttr} }` : `{ ${childrenAttr} }`;
      state.emitLine(`jsx(${tagJs}, ${allAttrs}${keyArg});`);
    } else {
      const childParts = el.children.map(c => emitChildToJs(c, state));
      const childrenAttr = `children: [${childParts.join(', ')}]`;
      const allAttrs = attrs.length > 0 ? `{ ${attrs.join(', ')}, ${childrenAttr} }` : `{ ${childrenAttr} }`;
...
function buildAttributes(el: Element): { parts: string[]; keyExpr: string | null } {
  const parts: string[] = [];
  ...
  // Extract key from parts for React 3rd-arg convention
  ...
  return { parts: filteredParts, keyExpr };
}
...
function emitChildToJs(node: Node, state: EmitState): string {
  ...
  if (node instanceof Element) {
    ...
    const attrResult = buildAttributes(node);
    const attrs = attrResult.parts;
    const keyArg = attrResult.keyExpr ? `, ${attrResult.keyExpr}` : '';

    if (hasChildren) {
      const childParts = node.children.map(c => emitChildToJs(c, state));
      const childrenAttr = childParts.length === 1 ? `children: ${childParts[0]}` : `children: [${childParts.join(', ')}]`;
      const fn = childParts.length === 1 ? 'jsx' : 'jsxs';
      const allAttrs = attrs.length > 0 ? `{ ${attrs.join(', ')}, ${childrenAttr} }` : `{ ${childrenAttr} }`;
...
function emitStatement(cf: ControlFlow, remaining: Node[], state: EmitState): void {
  ...
  if (state.directEmit) {
    for (const s of stmts) state.emitLine(s + ';');
  } else {
  ...
  if (cf.controlKind === 'for') {
    ...
    if (forMatch) {
      ...
      if (cf.children.length === 0) {
        ...
        state.output += 'jsxs(Fragment, { children: [';
        if (cf.children.length > 0) {
